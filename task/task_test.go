...
func TestTaskCreation(t *testing.T) {
	t1, err := task.NewTask("Walk", "Go for a walk", task.ImportantAndUrgent, 1*time.Hour)
...
func TestTaskCreationInvalidPriority(t *testing.T) {
	t1, err := task.NewTask("Walk", "Go for a walk", 5, 1*time.Hour)
	require.Error(t, err)
	assert.Nil(t, t1)
}

func TestTaskCreationInvalidPriorityToTime(t *testing.T) {
	t1, err := task.NewTask("Walk", "Go for a walk", 4, 1*time.Hour)
	require.Error(t, err)
	assert.Nil(t, t1)

	t2, err := task.NewTask("Walk", "Go for a walk", 1, 0)
	require.Error(t, err)
	assert.Nil(t, t2)
}
